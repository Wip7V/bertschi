...
use Dompdf\Dompdf;
      ...
      // $consulta1 = "SELECT distinct(ctdua.id_dua), ctdua.* FROM ctdua, ctpartida, ctcontenedor, partida_contenedor WHERE partida_contenedor.partida = ctpartida.id AND partida_contenedor.contenedor = ctcontenedor.id AND ctdua.id_dua = ctpartida.duae OR ctdua.id_dua = ctpartida.duas ORDER BY id_dua DESC";
       $consulta1 = "SELECT distinct ctdua.* FROM ctdua, ctpartida, ctcontenedor WHERE ctcontenedor.partida = ctpartida.id AND ctdua.id_dua = ctpartida.dua ORDER BY id_dua DESC";
        $consulta_mysql = mysql_query($consulta1) or die("mysql error: ".mysql_error() );

$html = file_get_contents("plantilla_cab.html");		
$html.="<h3>SALDO VIVO</H3>";
$html.="<div style='background-color: #000; height: 3px;'></div><br>";
            ...
            <tbody>
                 <tr>
                    ...
                    <td >";
                     if($item['entrada_salida']=='en') $html.="Entrada";
                     if($item['entrada_salida']=='in') $html.="Intermedio";
                     if($item['entrada_salida']=='sa') $html.="Salida";
                    $html.=" </td>
        ...
        ";
$sql2 = "SELECT ctpartida.*, ctdua.numero FROM ctpartida, ctdua WHERE ctpartida.dua = id_dua AND id_dua = ".$item['id_dua'];
        ...
        <table  >
        	...
        	<tbody>
        		";
                     ...
                     // $sql2 = "SELECT ctcontenedor.* FROM ctcontenedor, partida_contenedor WHERE partida_contenedor.contenedor = ctcontenedor.id AND partida_contenedor.partida = ".$item2['id'];
                     $sql2 = "SELECT ctcontenedor.* FROM ctcontenedor WHERE ctcontenedor.partida = ".$item2['id'];
                    // echo $sql2;
                     //if($item['entrada_salida']=='sa') $sql2 = "SELECT * FROM ctcontenedor WHERE duas = ".$item['id_dua'];
					 	...
					 	{ 
					 	$html.="<tr>";
					 	$html.="<td>".$item3['codigo']."</td>";
					 	$html.="<td>".$item3['fecha_entrada']."</td>";
					 	$html.="<td>".$item3['sector']."</td>";
					 	$html.="<td>".$item3['carril']."</td>";
					 	$html.="<td>".$item3['posicio']."</td>";
					 	$html.="<td>".$item3['pis']."</td>";
