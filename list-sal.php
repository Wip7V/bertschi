...
<div id="principal2">
    ...
    <div class="formulario">
    	<form method="post" id="formsal" action="list-sal.php" enctype="multipart/form-data">
        	Codigo DUA: 
        	  <select id="ndua_entrada" name="ndua_entrada" onchange="carga()">
        	<option value="0">Selecciona Codigo DUA</option>
<!--            <option value="">Mostrar todo</option>
-->           <?php 
			$sql="SELECT DISTINCT ndua_salida FROM ctdua WHERE ndua_salida!='' ORDER BY ndua_entrada";
			$result=mysql_query($sql) or die("error: ".mysql_error() );
			while($dades=mysql_fetch_array($result)){
				echo "<option value='".$dades['ndua_salida']."'>".$dades['ndua_salida']."</opcion> ";
			}
		?>	
         </select>
        <div id="imprimir">
	...
	$row="";
	if ($cont!="0"){
		$sqlsch="SELECT * FROM ctdua WHERE ndua_salida='".$cont."'";
	 }else{
		 $sqlsch="SELECT * FROM ctdua WHERE ndua_salida='0'";
